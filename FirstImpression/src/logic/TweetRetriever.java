...
public class TweetRetriever {
	...
	/*
	public List<Status> getAllTweetsTest(String query) {
		Query searchQuery = new Query("from:"+query);
	...
	*/
	...
	/**
	 * Retrieves Statuses that have recieved a number of favorites that is considered an outlier. Inner outliers are as defined as
	 * being less than the lower outer fence: Q1 - 1.5*IQ or greater than the upper outer fence: Q3 + 1.5*IQ where Q1 and Q3 are
	 * quartiles 1 and 3 and IQ is the Interquartile range. More information here: itl.nist.gov/div898/handbook/prc/section1/prc16.htm
	 * @param statuses The list of tweets to examine.
	 * @return A list containing tweets whose like counts were considered outliers by the above criteria.
	 */
	public List<Status> getLikeOutliers(List<Status> statuses)
	{
		...
		double thirdQuartile = getMedian(supplier.get().filter(integer -> integer > median).collect(Collectors.toList()));
		double firstQuartile = getMedian(supplier.get().filter(integer -> integer < median).collect(Collectors.toList()));
		double IQR = thirdQuartile - firstQuartile;
		//System.out.println(IQR);
		List<Status> lowExtremes = statuses.stream().filter(status -> status.getFavoriteCount() < (firstQuartile - 1.5 * IQR)).collect(Collectors.toList());
		List<Status> highExtremes = statuses.stream().filter(status -> status.getFavoriteCount() < (thirdQuartile + 1.5 * IQR)).collect(Collectors.toList());
		lowExtremes.addAll(highExtremes);
		return lowExtremes;
	}
	
	/**
	 * Retrieves tweets that are considered particularly popular for this group of tweets. This is implemented by returning tweets
	 * that have greater or equal the average number of likes.
	 * @param statuses The tweets to retrieve popular tweets from.
	 ...
	 * 
	 * @param statuses The list of statuses to retrieve from.
	 * @param allowOutliers whether to allow outliers in the calculation of average likes
	...
	/**
	 * Retrieves Statuses that have recieved a number of retweets that is considered an outlier. Outliers are as defined as
	 * being less than the lower outer fence: Q1 - 3*IQ or greater than the upper outer fence: Q3 + 3*IQ where Q1 and Q3 are
	 * quartiles 1 and 3 and IQ is the Interquartile range. More information here: itl.nist.gov/div898/handbook/prc/section1/prc16.htm
	 * @param statuses The list of tweets to examine.
	 * @return A list containing tweets whose like counts were considered outliers by the above criteria.
	 */
	public List<Status> getRetweetOutliers(List<Status> statuses)
	{
		...
		double thirdQuartile = getMedian(RetweetStream.filter(integer -> integer > median).collect(Collectors.toList()));
		double firstQuartile = getMedian(RetweetStream.filter(integer -> integer < median).collect(Collectors.toList()));
		double IQR = thirdQuartile - firstQuartile;
		List<Status> lowExtremes = statuses.stream().filter(status -> status.getRetweetCount() < (firstQuartile - 1.5 * IQR)).collect(Collectors.toList());
		List<Status> highExtremes = statuses.stream().filter(status -> status.getRetweetCount() < (thirdQuartile + 1.5 * IQR)).collect(Collectors.toList());
		lowExtremes.addAll(highExtremes);
		return lowExtremes;
	}
}
